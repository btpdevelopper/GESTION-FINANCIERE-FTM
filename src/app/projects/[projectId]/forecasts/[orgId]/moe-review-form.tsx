...
import { moeReviewForecastAction } from "@/server/forecast/forecast-actions";
...
export function MoeReviewForm({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await moeReviewForecastAction({ forecastId, projectId, decision, comment });
        router.refresh();
      } catch (err: unknown) {
    ...
    <form onSubmit={handleSubmit} className="space-y-3 rounded border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
        ...
        {(["APPROVED", "CORRECTION_NEEDED", "REFUSED"] as const).map((d) => (
          <label key={d} className="flex items-center gap-1.5 cursor-pointer">
            ...
            <span className="text-xs font-medium text-slate-700 dark:text-slate-300">
              {d === "APPROVED" ? "Approuver" : d === "CORRECTION_NEEDED" ? "Demander correction" : "Refuser"}
            </span>
          </label>
        ))}
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Votre commentaire sur ce prévisionnel…"
