...
const PHASES = [
  { id: FtmPhase.CREATION, label: "Création", color: "bg-slate-100 border-slate-200 dark:bg-slate-800/50 dark:border-slate-700" },
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-lg border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900/60">
        <div className="flex flex-1 min-w-[200px] items-center gap-2 rounded-md border border-slate-200 bg-slate-50 px-3 py-1.5 focus-within:ring-1 focus-within:ring-slate-400 dark:border-slate-700 dark:bg-slate-800">
          <Search className="h-4 w-4 text-slate-400" />
          <input
            type="text"
            placeholder="N° FTM, titre, initiateur..."
            value={search}
        ...
        <select
          ...
          className="rounded-md border border-slate-200 bg-white px-3 py-1.5 text-sm text-slate-700 outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200"
        >
          <option value="ALL">Sources (Toutes)</option>
          <option value="MOA">Demande MOA</option>
          <option value="MOE">Demande MOE</option>
        ...
        <select
          ...
          className="rounded-md border border-slate-200 bg-white px-3 py-1.5 text-sm text-slate-700 outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 max-w-[200px] truncate"
        >
          <option value="ALL">Entreprises (Toutes)</option>
        ...
        <select
          ...
          className="rounded-md border border-slate-200 bg-white px-3 py-1.5 text-sm text-slate-700 outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200"
        >
          <option value="NONE">Tri: Standard</option>
          <option value="DESC">Valeur estimée ⬇</option>
          <option value="ASC">Valeur estimée ⬆</option>
        </select>

        <label className="flex items-center gap-2 cursor-pointer rounded-md border border-slate-200 bg-white px-3 py-1.5 text-sm text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700">
          <input 
            type="checkbox" 
            checked={overdueOnly} 
          ...
          />
          <span className={`${overdueOnly ? "text-red-600 dark:text-red-400 font-medium" : ""}`}>
            Retards devis
          </span>
        </label>
      </div>
      ...
      <div className="flex h-[calc(100vh-16rem)] w-full gap-4 overflow-x-auto pb-4 pt-1 hide-scrollbar">
        {(isCompany ? COMPANY_PHASES : PHASES).map((col) => {
          const columnFtms = filteredFtms.filter((f) => {
            if (!isCompany) {
              if (col.id === "COMPLETED") return f.phase === FtmPhase.ACCEPTED || f.phase === FtmPhase.CANCELLED;
              ...
              if (col.id === "UPCOMING") {
                return f.phase === FtmPhase.CREATION || f.phase === FtmPhase.ETUDES;
              }
          ...
          return (
            <div
              ...
              className={`group flex shrink-0 flex-col rounded-xl p-3 shadow-sm ring-1 ring-slate-200/60 transition-all duration-300 ease-in-out dark:ring-slate-800/80 ${
                isCollapsed 
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-1 flex flex-col border-t border-slate-100 pt-2 dark:border-slate-800">
                        ...
                        {(ftm.phase === FtmPhase.QUOTING || ftm.phase === FtmPhase.ANALYSIS) && totalOrgs > 0 && (
