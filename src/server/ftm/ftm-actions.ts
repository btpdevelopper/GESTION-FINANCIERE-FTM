...
export async function createFtmAction(input: {
  ...
  lots: { lotLabel?: string; descriptionTravaux: string; organizationId: string; expectedResponseDate?: Date | null }[];
  requestedMoeResponseDate?: Date | null;
  documents?: { name: string; url: string; organizationId?: string | null }[];
}) {
  ...
  const ftm = await prisma.$transaction(async (tx) => {
    ...
    const record = await tx.ftmRecord.create({
      data: {
        ...
        requestedMoeResponseDate: input.requestedMoeResponseDate ?? null,
      },
    });

    const uniqueOrgs = Array.from(new Set(input.lots.map(l => l.organizationId)));
    for (const orgId of uniqueOrgs) {
      const lotData = input.lots.find(l => l.organizationId === orgId);
      await tx.ftmConcernedOrganization.create({
        data: { 
          ftmId: record.id, 
          organizationId: orgId,
          dateLimiteDevis: lotData?.expectedResponseDate ?? null,
        },
      });
    }
...
export async function submitQuoteAction(formData: FormData) {
  ...
  const ftmLotId = formData.get("ftmLotId") as string;
  const organizationId = formData.get("organizationId") as string;
  const amountHtCentsStr = formData.get("amountHtCents") as string;
  const file = formData.get("file") as File | null;

  if (!projectId || !ftmId || !ftmLotId || !organizationId || !amountHtCentsStr) {
    throw new Error("Paramètres manquants.");
  }
  const amountHtCents = BigInt(amountHtCentsStr);
  ...
  if (file && file.size > 0) {
    ...
    // Using the same securely isolated bucket
    const result = await uploadFtmDocument(ftmId, buffer, file.name, file.type);
...
export async function uploadFtmDocumentAction(formData: FormData) {
  ...
  if (ftm.phase !== FtmPhase.CREATION && ftm.phase !== FtmPhase.ETUDES) {
    throw new Error("L'upload de documents n'est possible que pendant la phase Études.");
  }

  const buffer = Buffer.from(await file.arrayBuffer());
  const { path } = await uploadFtmDocument(ftmId, buffer, file.name, file.type);
